...
@rem ! for /F "delims=" %%i in ('powershell Get-ExecutionPolicy') do set PERMISSION=%%i
@rem ! if "%PERMISSION%" == "Unrestricted" goto runPowerShell
goto runLegacy
...
start "Cassandra" "%JAVA_HOME%\bin\java" %JAVA_OPTS% %CASSANDRA_PARAMS% -cp %CASSANDRA_CLASSPATH% "%CASSANDRA_MAIN%"
goto finally
